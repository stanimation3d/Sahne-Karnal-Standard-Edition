...
// --- Karnal64 Sistem Çağrı Numaraları (örnek, gerçek değerler için karnal64.rs bakınız) ---
...
fn sys_resource_acquire(resource_id: &[u8], mode: u32) -> Result<u64, PowerError> {
    ...
    unsafe {
        asm!(
            "sc", // PowerPC syscall (replace with platform's syscall mechanism)
            in("r0") SYSCALL_RESOURCE_ACQUIRE,
            in("r3") id_ptr,
            in("r4") id_len,
            in("r5") mode,
            lateout("r3") ret,
...
fn sys_resource_release(handle: u64) -> Result<(), PowerError> {
    ...
    unsafe {
        asm!(
            "sc",
            in("r0") SYSCALL_RESOURCE_RELEASE,
            in("r3") handle,
            lateout("r3") ret,
            options(nostack)
        );
